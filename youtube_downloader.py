        ...
        with open(config_file, "r") as f:
            for line in f:
                if line.strip().startswith("Lang:"):
                    default_language = line.strip().split(":")[1].strip()
...
def change_language(language):
    ...
    window.title(messages.get('window_title', 'YouTube-Downloader') + f" | FFmpeg: {get_ffmpeg_version()}" + f" | {messages.get('downloads', 'Downloads')}: {download_counter}")
    update_button_texts()
...
def update_title():
    window.title(f"{messages.get('window_title', 'YouTube-Downloader')} | FFmpeg: {get_ffmpeg_version()} | {messages.get('downloads', 'Downloads')}: {download_counter}")
...
def update_button_texts():
    search_button.config(text=messages.get('search_button', 'Buscar'), background='white')
    directory_button.config(text=messages.get('directory_button', 'Cambiar Directorio'), background='white')
    download_button.config(text=messages.get('download_button', 'Descargar'), background='white')
    play_button.config(text=messages.get('play_button', 'Escuchar'), background='white')
    # Botón para editar la GUI
    edit_gui_button = tk.Button(window, text=messages.get('edit_gui_button', 'Edit GUI'), bg='white')
    edit_gui_button.grid(row=4, column=0, columnspan=3, padx=10, pady=(0, 10), sticky="ew")  # Ajustar padding y extender a lo ancho
    update_language_menu()
...
def edit_gui():
    ...
    for widget in [search_button, play_button, download_button, directory_button, format_combobox, entry, result_listbox, edit_gui_button]:
        widget.config(state=tk.DISABLED)

    # Crear la ventana de edición
    edit_window = tk.Toplevel(window)
    edit_window.title(messages.get('edit_gui_title', 'Edit GUI'))
    ...
    save_button = tk.Button(edit_window, text=messages.get('save_button', 'Guardar'), command=save_gui_state)
    save_button.pack()

    restore_button = tk.Button(edit_window, text=messages.get('restore_button', 'Restaurar'), command=restore_gui_state)
    restore_button.pack()

    # Botón para salir de la ventana de edición
    exit_button = tk.Button(edit_window, text=messages.get('exit_button', 'Salir'), command=exit_edit_gui)
    exit_button.pack()

def save_gui_state():
    save_column_widths()
    messagebox.showinfo(messages.get('save_info', 'Guardar'), messages.get('save_info', 'Configuración guardada exitosamente.'))
    exit_edit_gui()

def restore_gui_state():
    load_column_widths()
    messagebox.showinfo(messages.get('restore_info', 'Restaurar'), messages.get('restore_info', 'Configuración restaurada.'))
    exit_edit_gui()
...
settings_menu.add_cascade(label=messages.get('language_settings', 'Languages'), menu=language_submenu)
language_submenu.add_command(label=messages.get('language_name_es', 'Español'), command=lambda: change_language('es'))
language_submenu.add_command(label=messages.get('language_name_en', 'Inglés'), command=lambda: change_language('en'))
settings_menu.add_command(label=messages.get('check_updates', 'Check Updates'), command=check_ffmpeg_update)
settings_menu.add_command(label=messages.get('edit_gui_button', 'Edit GUI'), command=edit_gui)  # Nueva opción para editar GUI
...
search_button = tk.Button(window, text=messages.get('search_button', 'Buscar'), bg='white')  # Blanco
search_button.grid(row=0, column=1, padx=(3, 10), pady=10)  # Se ajusta el padding para el botón

# Botón para escuchar
play_button = tk.Button(window, text=messages.get('play_button', 'Escuchar'), bg='white')  # Blanco
play_button.grid(row=1, column=0, padx=10, pady=10)  # Se ajusta el padding para el botón

# Botón para iniciar la descarga
download_button = tk.Button(window, text=messages.get('download_button', 'Descargar'), bg='white')  # Blanco
...
format_combobox = ttk.Combobox(window, values=["mp3", "m4a", "opus", "vorbis", "wav", "webm"])
...
directory_button = tk.Button(window, text=messages.get('directory_button', 'Cambiar Directorio'), bg='white')  # Blanco
